...
fn repeated_label_first_definition_wins_endnote_and_repeat_renders_in_place() {
    ...
    // but visible, which beats GFM's silent drop. ADR-034 § render contract.
    let html = render("A[^1].\n\n[^1]: first\n\n[^1]: second\n");
    assert_eq!(html.matches(r#"<li id="fn-1">"#).count(), 1, "{html}");
    let (_, after_li) = html
...
// ---------------------------------------------------------------------------
// Task lists render as checkboxes (ADR-034 § Task lists, amended)
// ---------------------------------------------------------------------------
...
// (ADR-034), and `FootnoteIndex`'s collector stops at shortcode bodies — so a
