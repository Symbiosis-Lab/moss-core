...
//!   `newsletter.rs`'s email plain text (ADR-035) is the third.
//!
//!   **`build::page::meta::extract_description` deliberately does NOT use
//!   this policy**, despite being named alongside email in ADR-035's
...
//!   heading slug or hero overlay may reasonably want it. See ADR-035's
...
//!   new: nothing before ADR-035 walked the whole typed tree into a
...
/// Conventions (matching `newsletter.rs`'s pre-ADR-035 hand-rolled walker,
...
/// matching the pre-ADR-035 email walker's convention, which never wrote
...
fn render_block(
    ...
    match block {
        ...
        Block::Other(_) => {
            ...
            // lowering — matches the pre-ADR-035 walker, which never
            // arm'd `Event::Html`.
        }
    }
}
...
/// detection. The pre-ADR-035 walker never saw this: callouts didn't exist
...
fn render_inline_plain(inline: &Inline, index: &FootnoteIndex, out: &mut String) {
    match inline {
        ...
        Inline::Other(html) => {
            ...
            // plain text, matching the pre-ADR-035 walker (no Html arm).
        }
    }
}
