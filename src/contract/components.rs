...
pub const COMPONENTS: &[ComponentEntry] = &[
  ...
  <div class="moss-card-minimal">...</div>
    ...
    ComponentEntry {
        class: "moss-card-wrapper",
        kind: "instance",
        parent: "moss-cards",
        data_attrs: &[],
        example_html: r#"<div class="moss-card-wrapper"><div class="moss-card-overhead"><span class="moss-card-kicker">CHINA BOOKS REVIEW · 2026</span><a class="moss-card-permalink" href="/posts/foo/">★</a></div><a class="moss-card" href="https://outlet.example">...</a></div>"#,
        example_markdown: "",
        status: Status::Emerging,
        since: "1",
        description: "Outer container for linkblog cards (those whose child page has `external_url:`). Holds an `.moss-card-overhead` row above the card-anchor — kicker text + `★` archive link as inline siblings. Ordinary cards (no `external_url:`) stay as a bare `<a class=\"moss-card\">` without this wrapper.",
    },
    ComponentEntry {
        class: "moss-card-overhead",
        kind: "instance",
        parent: "moss-card-wrapper",
        data_attrs: &[],
        example_html: r#"<div class="moss-card-overhead"><span class="moss-card-kicker">CHINA BOOKS REVIEW · 2026</span><a class="moss-card-permalink" href="/posts/foo/">★</a></div>"#,
        example_markdown: "",
        status: Status::Emerging,
        since: "1",
        description: "Inline flex row above the card-anchor. Hosts the kicker (`<span class=\"moss-card-kicker\">`) and the `★` archive link as siblings on one baseline so the line reads as \"Publisher · Year ★\" — the kicker stays inside the row's text-flow rather than being pinned to the card's right edge. Only emitted for linkblog cards.",
    },
    ComponentEntry {
        class: "moss-card-permalink",
        kind: "instance",
        parent: "moss-card-overhead",
        data_attrs: &[],
        example_html: r#"<a class="moss-card-permalink" href="/posts/foo/" title="Permalink to 'Title'">★</a>"#,
        example_markdown: "",
        status: Status::Emerging,
        since: "1",
        description: "Author's-archive link mark (★, U+2605) inside `.moss-card-overhead`, rendered only when `external_url:` overrides the card's primary href. The card title links to the external canonical (publisher); the `★` links to the local archival copy at the page's slug. Semantically distinct from Daring-Fireball's linkblog ★ (which marks discussion permalink alongside commentary) — here the local copy is the same content preserved for resilience and stable bylines, not added commentary.",
    },
    ComponentEntry {
