...
pub const UNPREFIXED_LEGACY_CLASSES: &[&str] = &[
    ...
    "footer-default",
    "footer-link",
    // Surfaces the site JavaScript reads (declared 2026-08-09). Same reason as
    // the nav: these names were emitted, styled, and queried by name long before
    // the `moss-` convention, and a theme that targets them today would break if
    // they were renamed for tidiness.
    "container",
    "nav-content",
    "font-anchor",
    "font-trigger",
...
",
  ...
  <form class="moss-subscribe-form">...</form>
    ...
    ComponentEntry {
        class: "moss-subscribe-script",
        kind: "instance",
        parent: "",
        data_attrs: &[],
        example_html: r#"<script class="moss-subscribe-script">/* subscribe form handler */</script>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "Marks the `<script>` moss injects into `<head>` when a page carries a subscribe form. It is a handle, not a styling hook — a theme or a test uses it to find (or suppress) moss's own subscribe behaviour without matching on script contents.",
    },
    ComponentEntry {
  ...
  <a href="https://mosspub.com">
    ...
    ComponentEntry {
        class: "moss-breadcrumb-more",
        kind: "instance",
        parent: "",
        data_attrs: &[],
        example_html: r#"<button class="moss-breadcrumb-more" aria-expanded="false">…</button>"#,
        example_markdown: "",
        status: Status::Emerging,
        since: "0",
        description: "The masthead trail's counterpart to `.moss-nav-island-more`: stands in for the ancestor levels the trail folded, opens the levels menu on click, names them on hover via `data-tooltip`. Emitted (hidden) only when the trail has a middle to fold — three or more crumbs.",
    },
    ComponentEntry {
        class: "moss-breadcrumb-menu",
        kind: "instance",
        parent: "",
        data_attrs: &[],
        example_html: r#"<div class="moss-breadcrumb-menu" hidden>…</div>"#,
        example_markdown: "",
        status: Status::Emerging,
        since: "0",
        description: "Popover listing the masthead trail's folded ancestor levels, opened by `.moss-breadcrumb-more`. A sibling of `.nav-left` (which clips its own overflow), positioned against `.nav-content`. Same shape as `.moss-nav-island-menu`.",
    },
    ComponentEntry {
...
<li id="fn-1"><p>The note. <a class="moss-footnote-backref" href="#fnref-1" role="doc-backlink" aria-label="Back to reference 1">&#8617;&#xFE0E;</a></p>
    ...
    ComponentEntry {
        ...
        parent: "main-nav",
        data_attrs: &[],
        example_html: r#"<nav class="main-nav container"><div class="nav-content">…</div></nav>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "The row inside the nav bar that holds `.nav-left` and `.nav-right`. It is the box the responsive nav measures itself against: when the two groups no longer fit on one line, `nav-split.ts` sets `data-nav-split` on this element and the links move to a second row.",
    },
    ComponentEntry {
        class: "font-anchor",
        kind: "chrome",
        parent: "date-line",
        data_attrs: &[],
        example_html: r#"<div class="font-anchor"><button class="font-trigger size-std" aria-expanded="false"></button><div class="font-pill" id="fontPill">…</div></div>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "Positioning box for the reading-preferences control that sits at the end of an article's date line. Holds the trigger and the size pills; exists so the pills can be positioned against the trigger rather than the page.",
    },
    ComponentEntry {
        class: "font-trigger",
        kind: "instance",
        parent: "font-anchor",
        data_attrs: &[],
        example_html: r#"<button class="font-trigger size-std" aria-label="Reading preferences" aria-expanded="false" type="button"></button>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
    ...
    ComponentEntry {
        class: "cover-thumb",
        kind: "instance",
        parent: "moss-card-cover",
        data_attrs: &[],
        example_html: r#"<div class="moss-card-cover"><video src="clip.mp4" muted loop playsinline preload="metadata"></video><img src="clip.thumb.jpg" alt="A clip" class="cover-thumb" /></div>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "The still frame stacked over a video cover. It is what the reader sees until they hover: `card-video.ts` fades this image out and starts the `<video>` underneath, and fades it back in on leave. Emitted only for video covers, inside `.moss-card-cover` or `.moss-collection-cover`.",
    },
    ComponentEntry {
        class: "media-item",
        kind: "instance",
        parent: "",
        data_attrs: &[
            DataAttr { name: "data-type", values: &["image", "video", "iframe"], default: "image", description: "Which lightbox surface opens for this item." },
            DataAttr { name: "data-src", values: &[], default: "", description: "Full-size source the lightbox loads." },
            DataAttr { name: "data-title", values: &[], default: "", description: "Caption shown under the lightbox." },
            DataAttr { name: "data-article", values: &[], default: "", description: "URL of the article the item came from, linked from the caption." },
        ],
        example_html: r#"<figure class="media-item" tabindex="0" data-type="image" data-src="/img/full.webp" data-title="Kyiv" data-article="/posts/kyiv/">…</figure>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "One tile on the media-collection page. `fullscreen.ts` collects these in document order to build the lightbox playlist, so their order on the page is the order the arrows step through.",
    },
    ComponentEntry {
        ...
        parent: "",
        data_attrs: &[],
        example_html: r#"<div class="lightbox-content"><img class="lightbox-image" hidden /><video class="lightbox-video" controls hidden></video><iframe class="lightbox-iframe" hidden></iframe></div>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "The stage of the media-collection lightbox. Holds all three players at once; `fullscreen.ts` unhides whichever one matches the opened item's `data-type` and leaves the others hidden.",
    },
    ComponentEntry {
        class: "lightbox-image",
        kind: "instance",
        parent: "lightbox-content",
        data_attrs: &[],
        example_html: r#"<img class="lightbox-image" src="" alt="" hidden />"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "The lightbox's image player. Emitted empty and hidden; its `src` is filled in when an image item opens.",
    },
    ComponentEntry {
        ...
        parent: "lightbox-content",
        data_attrs: &[],
        example_html: r#"<video class="lightbox-video" controls hidden></video>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "The lightbox's video player. Emitted empty and hidden; paused and cleared when the lightbox closes so audio never outlives the overlay.",
    },
    ComponentEntry {
        ...
        parent: "lightbox-content",
        data_attrs: &[],
        example_html: r#"<iframe class="lightbox-iframe" hidden></iframe>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "The lightbox's embed surface, for media items that are an external player rather than a file.",
    },
    ComponentEntry {
        ...
        parent: "",
        data_attrs: &[],
        example_html: r#"<p class="lightbox-title"></p>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "Caption line under the lightbox stage. Filled from the open item's `data-title`.",
    },
    ComponentEntry {
        class: "lightbox-article-link",
        kind: "instance",
        parent: "",
        data_attrs: &[],
        example_html: r#"<a class="lightbox-article-link" href="">View in article →</a>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "The way back from a media tile to the article it appeared in. Its `href` is filled from the open item's `data-article`, and it is hidden when the item has none.",
    },
    ComponentEntry {
        ...
        parent: "",
        data_attrs: &[],
        example_html: r#"<button class="lightbox-close" aria-label="Close">&times;</button>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "Dismisses the media-collection lightbox. Escape does the same thing.",
    },
    ComponentEntry {
        class: "lightbox-next",
        kind: "instance",
        parent: "",
        data_attrs: &[],
        example_html: r#"<button class="lightbox-nav lightbox-next" aria-label="Next">&rsaquo;</button>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "Steps forward through the `.media-item` playlist, wrapping at the end. Carries `lightbox-nav` as well, which styles both arrows together.",
    },
    ComponentEntry {
        class: "lightbox-prev",
        kind: "instance",
        parent: "",
        data_attrs: &[],
        example_html: r#"<button class="lightbox-nav lightbox-prev" aria-label="Previous">&lsaquo;</button>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "Steps backward through the `.media-item` playlist, wrapping at the start.",
    },
    ComponentEntry {
        class: "comments-toggle",
        kind: "instance",
        parent: "moss-comments",
        data_attrs: &[],
        example_html: r#"<summary class="comments-toggle"><svg class="comments-icon">…</svg><span>3 comments</span><svg class="comments-chevron">…</svg></summary>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "The `<summary>` that opens and closes the comment thread. Its `<span>` holds the count, which the client rewrites as comments arrive — so the span is a contract of its own, not decoration.",
    },
    ComponentEntry {
        class: "comment-list",
        kind: "container",
        parent: "moss-comments",
        data_attrs: &[],
        example_html: r#"<ol class="comment-list"><li class="comment-item" …>…</li></ol>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
    ...
    ComponentEntry {
        ...
        parent: "comment-list",
        data_attrs: &[
            DataAttr { name: "data-comment-source", values: &["artalk"], default: "artalk", description: "Where the comment came from — moss's own server, or a syndicated platform." },
            DataAttr { name: "data-comment-id", values: &[], default: "", description: "Identifier within that source. Unique only per source, which is why nesting keys on the pair." },
        ],
        example_html: r#"<li class="comment-item" id="comment-artalk-12" data-comment-source="artalk" data-comment-id="12">…</li>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "One comment. Holds a `.comment-header`, a `.comment-body`, and — if it has replies — a nested `.comment-replies`. The `data-comment-*` pair is how the client matches a live comment to the one already on the page instead of rendering it twice.",
    },
    ComponentEntry {
        ...
        parent: "comment-item",
        data_attrs: &[],
        example_html: r#"<ol class="comment-replies"><li class="comment-item" …>…</li></ol>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "Nested replies under a comment, same shape as `.comment-list`. Emitted only when a comment has replies; the client creates one on demand when the first reply arrives.",
    },
    ComponentEntry {
        ...
        parent: "comment-item",
        data_attrs: &[
            DataAttr { name: "data-reply-id", values: &[], default: "", description: "The comment being replied to." },
            DataAttr { name: "data-reply-name", values: &[], default: "", description: "Display name of its author, used to prefill the form." },
        ],
        example_html: r#"<button type="button" class="comment-reply-btn" data-reply-id="12" data-reply-name="Yi">↩︎ Reply</button>"#,
        example_markdown: "",
        status: Status::Confirmed,
        since: "0",
        description: "Moves the comment form under this comment so the reply is written where it will appear. Emitted only for comments moss can reply to; a syndicated comment gets a link out to its own platform instead.",
    },
];
