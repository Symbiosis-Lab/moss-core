...
pub const COMPONENTS: &[ComponentEntry] = &[
  ...
  <div class="moss-card-minimal">...</div>
    ...
    ComponentEntry {
        ...
        parent: "moss-cards",
        data_attrs: &[],
        example_html: r#"<div class="moss-card-wrapper"><a class="moss-card" href="https://outlet.example">...</a><a class="moss-card-permalink" href="/posts/foo/">★</a></div>"#,
        example_markdown: "",
        status: Status::Emerging,
        since: "1",
        description: "Layout sibling-pair for linkblog cards. Wraps a `.moss-card` (whose href targets the external outlet) and an adjacent `.moss-card-permalink` (★) pointing to the local archive. Only emitted when the child page has `external_url:` set — ordinary cards stay as a bare `<a class=\"moss-card\">`. Two siblings instead of nested anchors so the HTML stays valid. CSS uses `position: relative` here so the `★` can be absolutely positioned within the card's bounding box.",
    },
    ComponentEntry {
        ...
        status: Status::Emerging,
        since: "1",
        description: "Linkblog permalink mark (★, U+2605). Sibling to `.moss-card` inside `.moss-card-wrapper`; rendered only when `external_url:` overrides the card's primary href. Links to the local archive URL. Convention follows Daring Fireball (2002): title → external canonical, ★ → local permalink.",
    },
    ComponentEntry {
